// ════════════════════════════════════════════
//  VYBE — app.js (Supabase Storage pour médias)
// ════════════════════════════════════════════
...
//  Limites : images/audio 50MB, vidéos 200MB
// ════════════════════════════════════════════
...
async function loadStories(){
  ...
  storiesEl.innerHTML=Object.values(byAuthor).map(g=>{
    ...
    return `<div class="story-item" onclick="openStoryViewer('${u.id}')">
      <div class="story-avatar" style="background:${avatarColor(u.username)};">${avatarInit(u.name)}</div>
...
window.previewStory=(input,type)=>{
  ...
  pendingStory={file,type};
  const url=URL.createObjectURL(file);
  const prev=document.getElementById('story-preview');
  prev.style.display='block';
  prev.innerHTML=type==='image'
    ?`<img src="${url}" style="width:100%;max-height:300px;object-fit:contain;display:block;border-radius:12px;background:#000;"/>`
    :`<video src="${url}" controls style="width:100%;max-height:300px;display:block;border-radius:12px;"></video>`;
};
...
window.createStory=async()=>{
  ...
  try {
    ...
    if(pendingStory){
      ...
      mediaUrl=await uploadFile(pendingStory.file,'stories');
      mediaType=pendingStory.type;
    }
    const{error}=await sb.from('stories').insert({author_id:currentUser.id,media_url:mediaUrl,media_type:mediaType,text});
    if(error)throw new Error(error.message);
...
function showStory(idx){
  ...
  const s=storyList[idx];const u=s.profiles;
  ...
  if(s.media_url){
    if(s.media_type==='image')content.innerHTML=`<img src="${s.media_url}" style="max-width:100%;max-height:80vh;border-radius:12px;object-fit:contain;"/>`;
...
window.nextStory=()=>{if(storyIndex<storyList.length-1){storyIndex++;showStory(storyIndex);}else closeStoryViewer();};
window.prevStory=()=>{if(storyIndex>0){storyIndex--;showStory(storyIndex);}};
...
window.previewMedia=(input,type)=>{
  ...
  const sizeInfo=`<div style="color:var(--muted);font-size:12px;padding:6px 10px;">📁 ${(file.size/1024/1024).toFixed(1)} MB</div>`;
  if(type==='image')prev.innerHTML=`<img src="${url}" style="width:100%;max-height:300px;object-fit:contain;display:block;background:#000;"/>${rm}`;
  else if(type==='video')prev.innerHTML=`<video src="${url}" controls style="width:100%;max-height:300px;display:block;"></video>${sizeInfo}${rm}`;
  else prev.innerHTML=`<audio src="${url}" controls style="width:100%;padding:12px;display:block;"></audio>${sizeInfo}${rm}`;
};
window.removeMedia=()=>{
  pendingMedia=null;
  const p=document.getElementById('media-preview');p.style.display='none';p.innerHTML='';
  ['media-image','media-video','media-audio'].forEach(id=>{const el=document.getElementById(id);if(el)el.value='';});
};
...
window.createPost=async()=>{
  ...
  try {
    ...
    if(pendingMedia){
      ...
      mediaUrl=await uploadFile(pendingMedia.file,'posts');
      mediaType=pendingMedia.type;
    }
    const{error}=await sb.from('posts').insert({
      author_id:currentUser.id,body:text,media_url:mediaUrl,media_type:mediaType
    });
    if(error)throw new Error(error.message);
    document.getElementById('post-input').value='';
...
window.previewReel=(input)=>{
  ...
  prev.style.display='block';
  prev.innerHTML=`<video src="${url}" controls style="width:100%;max-height:300px;display:block;border-radius:12px;"></video>
  <div style="color:var(--muted);font-size:12px;padding:8px;">📁 ${(file.size/1024/1024).toFixed(1)} MB</div>`;
};
...
window.createReel=async()=>{
  ...
  try {
    const videoUrl=await uploadFile(pendingReel,'reels');
    const{error}=await sb.from('reels').insert({author_id:currentUser.id,video_url:videoUrl,caption});
    if(error)throw new Error(error.message);
...
function postHTML(p,likedSet){
  ...
  return `<div class="post-card">
    ...
    <div class="post-actions">
      ...
      ${u.id!==currentUser.id?`<button class="action-btn" onclick="openChat('${u.id}','${u.username}','${esc(u.name)}')">
        ...
        Message</button>`:''}
      ${u.id===currentUser.id?`<button class="action-btn delete-btn" onclick="deletePost('${p.id}',this)">
...
async function loadReels(){
  ...
  listEl.innerHTML=loading();
  const{data:reels}=await sb.from('reels').select('*,profiles:author_id(id,name,username)').order('created_at',{ascending:false});
  if(!reels||!reels.length){
    listEl.innerHTML=`<div class="empty-feed"><h3>Aucun reel pour l'instant</h3><p>Publie le premier depuis ton profil !</p></div>`;return;
  }
  listEl.innerHTML=reels.map(r=>{
    ...
    return `<div class="reel-card">
      <video src="${r.video_url}" controls preload="metadata" style="width:100%;max-height:500px;display:block;"></video>
      <div class="reel-overlay">
        <div class="reel-author">
          <div class="avatar" style="background:${avatarColor(u.username)};width:32px;height:32px;font-size:12px;cursor:pointer" onclick="viewProfile('${u.id}')">${avatarInit(u.name)}</div>
          <div class="reel-author-name" onclick="viewProfile('${u.id}')">${esc(u.name)}</div>
        </div>
        ${r.caption?`<div class="reel-caption">${esc(r.caption)}</div>`:''}
        <div class="reel-actions">
          <button class="reel-btn-action ${liked?'liked':''}" onclick="likeReel('${r.id}',this)">❤️ <span id="rlc-${r.id}">${r.likes_count??0}</span></button>
...
window.likeReel=async(reelId,btn)=>{
  ...
  const el=document.getElementById('rlc-'+reelId);if(el)el.textContent=newLikes.length;
};
window.deleteReel=async(reelId,btn)=>{
  if(!confirm('Supprimer ce reel ?'))return;
  btn.disabled=true;
  await sb.from('reels').delete().eq('id',reelId);
  btn.closest('.reel-card')?.remove();showToast('Reel supprimé.');
};
...
window.toggleLike=async(postId,btn)=>{
  ...
  const svg=btn.querySelector('svg');if(svg)svg.setAttribute('fill',btn.classList.contains('liked')?'currentColor':'none');
};
window.repost=async(postId)=>{
  ...
  const el=document.getElementById('rc-'+postId);if(el)el.textContent=parseInt(el.textContent||'0')+1;
...
//  COMMENTAIRES
// ════════════════════════════════════════════
window.openComments=async(postId)=>{activeCommentPostId=postId;await loadComments();document.getElementById('comments-modal').classList.add('open');};
window.closeComments=()=>{document.getElementById('comments-modal').classList.remove('open');activeCommentPostId=null;};
async function loadComments(){
  const listEl=document.getElementById('comments-list');listEl.innerHTML=loading();
  const{data:comments}=await sb.from('comments').select('*,profiles:author_id(name,username)').eq('post_id',activeCommentPostId).order('created_at',{ascending:true});
...
window.submitComment=async()=>{
  ...
  const el=document.getElementById('cc-'+activeCommentPostId);if(el)el.textContent=parseInt(el.textContent||'0')+1;
};
...
async function renderUserList(q){
  ...
  if(!filtered.length){listEl.innerHTML=`<div class="no-results">${allUsers.length===0?'Aucun autre utilisateur inscrit':'Aucun résultat pour "'+q+'"'}</div>`;return;}
  const{data:follows}=await sb.from('follows').select('following_id').eq('follower_id',currentUser.id);
...
async function loadProfile(userId){
  ...
  if(userId===currentUser.id){
    postActions.style.display='flex';btn.textContent='Modifier';btn.onclick=openEditModal;
  } else {
...
window.openFollowModal=async(type)=>{
  ...
  listEl.innerHTML=users.map(u=>{
    const isF=followSet.has(u.id);const isMe=u.id===currentUser.id;
    return `<div class="conv-item" onclick="closeFollowModal();viewProfile('${u.id}')">
      ...
      <div class="conv-info"><div class="conv-name">${esc(u.name)}</div><div class="conv-preview">@${u.username}${u.bio?' · '+u.bio:''}</div></div>
      ${!isMe?`<button class="follow-btn ${isF?'following':'not-following'}" onclick="event.stopPropagation();toggleFollow('${u.id}');this.textContent=this.classList.contains('following')?'Suivre':'Abonné';this.className='follow-btn '+(this.classList.contains('following')?'not-following':'following');">${isF?'Abonné':'Suivre'}</button>`:''}
    </div>`;
  }).join('');
};
...
async function loadConversations(){
  ...
  const{data:allMsgs}=await sb.from('messages').select('*').or(`sender_id.eq.${currentUser.id},receiver_id.eq.${currentUser.id}`).order('created_at',{ascending:false});
  listEl.innerHTML=users.map(u=>{
...
async function renderChatMessages(){
  const uid=activeChatWith.id;const chatEl=document.getElementById('chat-messages');
...
window.sendChatMedia=async(input)=>{
  ...
  const btn=document.querySelector('.chat-media-btn');
  if(btn)btn.style.opacity='.5';
  showToast('Upload en cours...');
  try {
    const type=getMediaType(file);
    const mediaUrl=await uploadFile(file,'chat');
    await sb.from('messages').insert({sender_id:currentUser.id,receiver_id:activeChatWith.id,body:'',media_url:mediaUrl,media_type:type});
    input.value='';
  } catch(e){showToast('Erreur : '+e.message);}
