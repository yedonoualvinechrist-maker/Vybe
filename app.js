...
window.register = async () => {
  ...
  // Si email fourni → on l'utilise directement (permet récupération)
  // Sinon → email interne invisible
...
window.showForgotPassword = (show = true) => {
  ...
  // cacher les tabs quand on est sur forgot
...
window.sendResetEmail = async () => {
  ...
  // Vérifier que cet email existe dans profiles
  const { data: profile } = await sb.from('profiles')
    .select('id').eq('internal_email', email).maybeSingle();

  if (!profile) {
    errEl.textContent = 'Aucun compte trouvé avec cet email.'; return;
  }
